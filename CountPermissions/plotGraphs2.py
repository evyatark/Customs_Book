...
def filter_lines_containing_this_code(df, regulation):
...
def howManyItemsHaveRegulation(onlyItemsWithDeclarations = False):
    ...
    results = []
    #regs = sorted( [x for x in all_codes if x.startswith('02')] )
    regs = sorted(all_codes)
    for reg in regs:  #  regulations:
        results.append(howManyItemsHaveRegulation1(file_df, reg, onlyItemsWithDeclarations))
    return regs, results
...
def howManyItemsHaveRegulation2(regulations):
    ...
    for reg in regulations:
        results.append(howManyItemsHaveRegulation1(file_df, reg))
    return results


def howManyItemsHaveRegulation1(df, regulation, include_onlY_items_with_declarations = False):
    """
    regulation is the code, such as '0102'
    ...
    df["code"] = ''     # add column named "code"
    for ind in df.index:
        list_of_regulations_as_str = df.at[ind, COLUMN_NAME_ISHURIM_LIST]
        v = literal_eval(list_of_regulations_as_str)
        # v is a list of codes
        df.at[ind, "code"] = regulation in v

    # filter_lines_containing_ value True in column named "code"
    condition = df["code"] == True
...
def howManyCustomsItemsNeedEachRegulation(onlyItemsWithDeclarations = False):
    codes, counts = howManyItemsHaveRegulation(onlyItemsWithDeclarations)
...
def show_plot(plt, series1, series2, xmin, xmax):
    pp1 = plt.barh(series1.index[xmin:xmax], series1.values[xmin:xmax], label='כולל ללא הצהרות יבוא'[::-1]) #, height=0.9)
    pp2 = plt.barh(series2.index[xmin:xmax], series2.values[xmin:xmax], label='רק עם הצהרות יבוא'[::-1]) # , color='g', height=0.9)
    add_titles(plt)
    plt.show()
...
def do_plot2():
    ...
    for bar in pl:
        ...
        plt.annotate(str(int(percent)) + "%",  #            bar.get_height(),
            xy=(bar.get_x() - 0.07, bar.get_height() + 10), fontsize=8)
        index = index + 1
