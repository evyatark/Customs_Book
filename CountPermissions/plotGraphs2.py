...
def add_count_ishurim(file_df):
    ...
    :param file_df:
    :return: dataframe after adding the values
    """
...
def howManyItemsHaveThisSpecificRegulation_and_only_agents_or_importers(df1, regulation_code, include_only_items_with_agents = False, include_only_items_with_importers = False):
    df = addColumnWithCode(df1, regulation_code)
    # filter the df to retain only lines containing value True in column named "code"
    # (and possibly only if they have a number > 0 in column COLUMN_NAME_NUMBER_OF_DECLARATIONS)
    condition = df["code"] == True
    if include_only_items_with_agents:
        condition = (df["code"] == True) & (df[COLUMN_NAME_NUMBER_OF_AGENTS] > 0)
    elif include_only_items_with_importers:
...
def do_plot_customs_items_requiring_each_regulation_for_declarations(calculate_from_excel_file = True, use_percents = True):
    ...
    #legend_str_without, legend_str_with = ( "כולל ללא סוכני מכס", "רק עם סוכני מכס" )
    #legend_str_without, legend_str_with = ( "כולל ללא יבואנים", "רק עם יבואנים" )
    plot_these_data(d1, d2, legend_str_without, legend_str_with, use_percents)


def do_plot_customs_items_requiring_each_regulation_for_agents(calculate_from_excel_file = True, use_percents = True):
    d1 = {}
    d2 = {}
    if calculate_from_excel_file:
        d1 = howManyCustomsItemsNeedEachRegulation2({'declarations': False, 'agents': False, 'importers': False})
        d2 = howManyCustomsItemsNeedEachRegulation2({'declarations': False, 'agents': True, 'importers': False})
    #legend_str_without, legend_str_with = ( "כולל ללא הצהרות יבוא", "רק עם הצהרות יבוא" )
    legend_str_without, legend_str_with = ( "כולל ללא סוכני מכס", "רק עם סוכני מכס" )
    #legend_str_without, legend_str_with = ( "כולל ללא יבואנים", "רק עם יבואנים" )
    plot_these_data(d1, d2, legend_str_without, legend_str_with, use_percents)
...
def do_plot_customs_items_requiring_each_regulation_for_importers(calculate_from_excel_file = True, use_percents = True):
    d1 = {}
    d2 = {}
    if calculate_from_excel_file:
        d1 = howManyCustomsItemsNeedEachRegulation2({'declarations': False, 'agents': False, 'importers': False})
        d2 = howManyCustomsItemsNeedEachRegulation2({'declarations': False, 'agents': False, 'importers': True})
    #legend_str_without, legend_str_with = ( "כולל ללא הצהרות יבוא", "רק עם הצהרות יבוא" )
    #legend_str_without, legend_str_with = ( "כולל ללא סוכני מכס", "רק עם סוכני מכס" )
    legend_str_without, legend_str_with = ( "כולל ללא יבואנים", "רק עם יבואנים" )
    plot_these_data(d1, d2, legend_str_without, legend_str_with, use_percents)
...
def main():
    ...
    do_plot_customs_items_requiring_each_regulation_for_agents(use_percents=False)
    do_plot_customs_items_requiring_each_regulation_for_importers(use_percents=False)
