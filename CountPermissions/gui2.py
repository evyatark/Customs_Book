...
from gui3 import CalcInterface
...
def click_to_process():
    print("button clicked")
    ## calling this blocks the mainloop!! ToDo fix this
    calcInterface.do1(myApp.plotView)
...
def add_notebook_in_frame(container):
    ...
    button = ttk.Button(tab_1, text='Click to process data...', command=click_to_process)
...
def createCanvasPlotView(container, *_):
    plotView = CanvasPlotView(container)
    ...
    plotView.grid(row=0, column=0, sticky='nsew', padx=5, pady=5)
    container.columnconfigure(0, weight=1)
    container.rowconfigure(0, weight=1)
...
def draw_some_plot(plotView, legend_str_without, legend_str_with):
    d1, d2 = get_hardcoded_values()
...
class MyApplication(tk.Tk):
    ...
    def __init__(self, calcInterface,  *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.calcInterface = calcInterface
...
#                          , style='TFrame')
        ...
        #self.update()
        self.plotView = createCanvasPlotView(notebook_frame_2)
...
if __name__ == "__main__":
    ...
    myApp = MyApplication(calcInterface)
    #myApp.calcInterface.do1(myApp.plotView)
